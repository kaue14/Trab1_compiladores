...
static char* internar_string(const char *texto) {
    Node *atual = tabela_strings;

    while (atual != NULL) {
        if (strcmp(atual->str, texto) == 0) {
            return atual->str;
        }
        atual = atual->next;
    }
    ...
    novo_no->str = strdup(texto);

    novo_no->next = tabela_strings;
    tabela_strings = novo_no;
...
static void guarda_lexema(void) {
    microc_yylval.symbol = internar_string(yytext);
}
...
%%
  ...
  * confundido com o fim do arquivo pelo main() de teste abaixo. 
    MUDEI ESTA PARTE POIS NÃO ESTAVA RECONHECENDO O ERRO DE ABRIR COMENTARIO E NÃO FECHAR*/
...
\'([^\'\\\n]|\\.)\' {
                        ...
                        if (yytext[1] == '\\') {
                            switch(yytext[2]) {
                                case 'n': caracter = '\n'; break;
                                case 't': caracter = '\t'; break;
                                case '\\': caracter = '\\'; break;
                                case '\'': caracter = '\''; break;
                                case '0': caracter = '\0'; break;
                                default: caracter = yytext[2]; break;
                            }
                        } else {
                            caracter = yytext[1];
                        }

                        char buffer[2] = {caracter, '\0'};
                        
                        microc_yylval.symbol = internar_string(buffer);
                        return CHARCONST;
                    }

\'[^\'\n]+\'        {
                        microc_yylval.error_msg = "Erro: maior que 1 caracter";
                        return UNDEF;
                    }

\'[^\'\n]*          {
                        microc_yylval.error_msg = "Erro: falta fechar aspas";
                        return UNDEF;
                    }
